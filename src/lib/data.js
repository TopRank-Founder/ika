...
const getStorage = () => typeof window !== 'undefined' ? localStorage : { getItem: () => null, setItem: () => null };

export const DataStore = {
  // ── Get collection ──
  get(collection) {
    try {
      const data = getStorage().getItem(`ika_${collection}`);
  ...
  save(collection, data) {
    getStorage().setItem(`ika_${collection}`, JSON.stringify(data));
  },
  ...
  getSetting(key) {
    try {
      const settings = JSON.parse(getStorage().getItem('ika_settings') || '{}');
      return settings[key];
    } catch { return null; }
  },
  ...
  setSetting(key, value) {
    try {
      const settings = JSON.parse(getStorage().getItem('ika_settings') || '{}');
      settings[key] = value;
      getStorage().setItem('ika_settings', JSON.stringify(settings));
    } catch { /* ignore */ }
  },

  getSettings() {
    try {
      return JSON.parse(getStorage().getItem('ika_settings') || '{}');
    } catch { return {}; }
  },
  ...
  isInitialized() {
    return getStorage().getItem('ika_initialized') === 'true';
  },

  markInitialized() {
    getStorage().setItem('ika_initialized', 'true');
  }
};
